...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/tiaragajjar/Desktop/lab/EXP6 word priming/Word Priming _lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[studytext, studyslider],
        )
    ...
    for thisTrial_2 in trials_2:
        ...
        test = data.Routine(
            name='test',
            components=[test_text, textbox, submit_text, testmouse],
        )
        test.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        test_text.setText(test_words)
        textbox.reset()
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            if test_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > test_text.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    test_text.tStop = t  # not accounting for scr refresh
                    test_text.tStopRefresh = tThisFlipGlobal  # on global time
            ...
            if submit_text.status == STARTED:
                ...
                if tThisFlipGlobal > submit_text.tStartRefresh + 10-frameTolerance:
                    ...
                    submit_text.tStopRefresh = tThisFlipGlobal  # on global time
            ...
            # if testmouse is stopping this frame...
            if testmouse.status == STARTED:
                ...
                if tThisFlipGlobal > testmouse.tStartRefresh + 10-frameTolerance:
                    ...
                    testmouse.tStopRefresh = tThisFlipGlobal  # on global time
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        if not gotValidClick:
                            testmouse.clicked_name.append(None)
