...
expName = 'animal words'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/tiaragajjar/Desktop/lab/solo porject/animal words_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    start_l_rect = visual.Rect(
        win=win, name='start_l_rect',
        width=(0.5, 0.5)[0], height=(0.5, 0.5)[1],
        ori=0.0, pos=(-0.4, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
        opacity=None, depth=0.0, interpolate=True)
    start_r_rect = visual.Rect(
        win=win, name='start_r_rect',
        width=(0.5, 0.5)[0], height=(0.5, 0.5)[1],
        ori=0.0, pos=(0.4, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    star = visual.ShapeStim(
        ...
        lineWidth=1.0,
        colorSpace='rgb', lineColor='yellow', fillColor='yellow',
    ...
    for thisTrial in trials:
        ...
        start = data.Routine(
            name='start',
            components=[start_l_rect, start_r_rect, start_l_image, start_r_image, star, start_mouse],
        )
        ...
        while continueRoutine:
            ...
            # *start_l_rect* updates
            
            # if start_l_rect is starting this frame...
            if start_l_rect.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                start_l_rect.frameNStart = frameN  # exact frame index
                start_l_rect.tStart = t  # local t and not account for scr refresh
                start_l_rect.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(start_l_rect, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'start_l_rect.started')
                # update status
                start_l_rect.status = STARTED
                start_l_rect.setAutoDraw(True)
            
            # if start_l_rect is active this frame...
            if start_l_rect.status == STARTED:
                # update params
                pass
            
            # *start_r_rect* updates
            
            # if start_r_rect is starting this frame...
            if start_r_rect.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                start_r_rect.frameNStart = frameN  # exact frame index
                start_r_rect.tStart = t  # local t and not account for scr refresh
                start_r_rect.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(start_r_rect, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'start_r_rect.started')
                # update status
                start_r_rect.status = STARTED
                start_r_rect.setAutoDraw(True)
            
            # if start_r_rect is active this frame...
            if start_r_rect.status == STARTED:
                # update params
                pass
        ...
        trial = data.Routine(
            name='trial',
            components=[right_rectangle, left_rectangle, left_image, right_image, trial_sound, trial_mouse],
        )
        ...
        trial_sound.setSound(target_sound, secs=1.59, hamming=True)
        ...
        while continueRoutine:
            ...
            if trial_sound.status == STARTED:
                ...
                if tThisFlipGlobal > trial_sound.tStartRefresh + 1.59-frameTolerance or trial_sound.isFinished:
